...
public class LogComponentManipulator implements Manipulator {
    ...
    public LogComponentManipulator(Class<?> useClass, Manipulation manipulation, Supplier<Boolean> shouldUseTuning, RawNodeFactory factory, IMattLogger logger) {
    ...
    public static void check(Method method) {
        ...
        if (quantity > 1) {
            throw Exceptions.TOO_MANY_ANNOTATIONS(methodName, simpleName);
        }

        if (method.getAnnotations().length == 0) {
            throw Exceptions.NO_ANNOTATIONS_ON_METHOD(methodName, simpleName);
        }

        if ((conf != null || tune != null) && method.getParameterCount() != 0) {
            throw Exceptions.BAD_CONF_OR_TUNE(methodName, simpleName);
        }

        if (log != null && method.getParameterCount() == 0) {
            throw Exceptions.BAD_LOG(methodName, simpleName);
        }
